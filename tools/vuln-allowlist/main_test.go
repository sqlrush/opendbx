...
func TestRun_CalledValidExemption(t *testing.T) {
	t.Parallel()
	allow := writeAllowlist(t, `{"exemptions":[
		{"osv_id":"GO-2026-4602","module":"stdlib","expiry":"2026-08-14","reason":"r","spec_ref":"spec-0.9 § 3.1"}
	]}`)
	stream := `{"finding":{"osv":"GO-2026-4602","trace":[
		{"module":"stdlib","function":"os.ReadDir"},
...
func TestRun_MixedExemption(t *testing.T) {
	...
	stream := strings.Join([]string{
		`{"finding":{"osv":"GO-2026-AAAA","trace":[{"function":"f"},{"function":"main"}]}}`,
		`{"finding":{"osv":"GO-2026-BBBB","trace":[{"function":"f"},{"function":"main"}]}}`,
	}, "\n")
	var out bytes.Buffer
	code := run(allow, strings.NewReader(stream), &out, fixedTime(t))
	if code != 1 {
...
func TestRun_DuplicateFindings(t *testing.T) {
	...
	stream := strings.Repeat(
		`{"finding":{"osv":"GO-2026-CCCC","trace":[{"function":"f"},{"function":"main"}]}}`+"\n",
		3)
	var out bytes.Buffer
	code := run(allow, strings.NewReader(stream), &out, fixedTime(t))
	if code != 0 {
	...
	// Count occurrences of [exempt] — should be exactly 1 even with 3 dup findings.
...
func TestShippedAllowlist_Parses(t *testing.T) {
	...
	for id, ex := range list {
		...
		if exp, err := time.Parse("2006-01-02", ex.Expiry); err != nil {
			t.Errorf("%s expiry parse: %v", id, err)
