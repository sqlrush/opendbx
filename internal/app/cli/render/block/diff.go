...
// Diff is the spec-0.13 stub for the diff block type. Render returns
// (nil, ErrUnsupportedNode) per the spec-0.13 D-3 contract (replaces R1
// panic path; R2 codex HIGH-5).
//
// TODO(spec-1.13): replace stub Render with real implementation; see spec-1.7 D-6 R2 D6 deferred-stub contract.
type Diff struct{}

// Render satisfies the RenderNode interface but returns the unsupported
// sentinel for this stub.
func (Diff) Render(_ Context) (buffer.Buffer, error) {
