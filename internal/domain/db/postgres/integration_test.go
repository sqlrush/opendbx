...
func TestIntegrationBadHost(t *testing.T) {
	dsnOrSkip(t) // ensure the suite is enabled
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	_, err := db.Open(ctx, "postgres", "postgres://u:p@127.0.0.1:1/db?sslmode=disable&connect_timeout=2")
	if err == nil {
...
func TestIntegrationUnknownDatabase(t *testing.T) {
	dsnOrSkip(t)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	// A database that does not exist yields SQLSTATE 3D000 → DB.CONNECT_FAILED.
	_, err := db.Open(ctx, "postgres", "postgres://u:p@127.0.0.1:1/nonexistent_db_zzz?sslmode=disable&connect_timeout=2")
	if err == nil {
		t.Fatal("Open unknown database returned nil error")
	}
