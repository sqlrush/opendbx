...
- **version choice (v5.7.6 not latest v5.8.0)**: v5.8.0 bumps `stretchr/testify` to v1.11.1 and `gopkg.in/check.v1` to v1.0.0; under the project's go-version-locked `go mod tidy`, that closure pulls `check.v1 → kr/pretty → rogpeppe/go-internal v1.15.0` (which declares `go 1.25`), forcing the project go directive 1.24.0 → 1.25. (v5.8.0's OWN go.mod declares `go 1.24.0` — the bump is transitive via the check.v1/go-internal TEST chain, capped by the `go.mod exclude go-internal v1.15.0`, not by v5.8.0 directly.) v5.7.6 requires only `testify v1.8.1`, so MVS keeps the existing `testify v1.8.4` + the go-1.24-compatible closure — no go bump. API for opendbx's surface (`pgxpool.New`/`Ping`/`QueryRow`/`pgconn.Timeout`/`*pgconn.PgError`) identical across 5.7/5.8. **CVE note (post-impl T-10a)**: v5.7.6 carries GO-2026-4772/4771 (fixed in v5.9.0); v5.9.0's go.mod declares `go 1.25.0` so the fix forces the same go bump → kept v5.7.6, vuln-allowlisted (not reachable, package-result), revisit at the Stage 2+ go-1.25 toolchain bump.
