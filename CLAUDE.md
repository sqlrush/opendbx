...
### 规则 A14：UI 5 层 Review + 共用组件 + 回答前 grep
- **UI（5 层 pipeline，落地 spec-0.11.5）**：
  - L1 静态不变量（unit）：runewidth 行宽 / ANSI 嵌套合法
  - L2 PTY + cell golden（integration）：`creack/pty` + `hinshun/vt10x`
  - L3 视觉 golden + 像素 diff（E2E）：`charmbracelet/freeze` + `pixelmatch`
  - L4 AI 视觉评审（label 触发，不阻 merge）：本地 Qwen2.5-VL-72B（llama.cpp on M5 Max）
  - L5 真终端手工（merge 前）：iTerm2 / Alacritty / kitty × 120 / 80 / 中文混排
- **硬约束**：`runewidth.EastAsianWidth = false`、表格用统一 builder（`internal/app/cli/components/table.go`）。headless 通过 ≠ UI 通过。完整规范见设计仓 CLAUDE.md § 3.9
- **共用组件**：修改 `render/block/*` / `components/picker.go` / `render/scrollback/*` / `render/streaming/*` 前必 grep 列出所有调用方，逐一验证。PR 描述强制列"调用方清单 + 各调用方 5 层验证结果"
...
7.5. ❌ UI 改动只过 cell-level golden 不跑视觉 / AI review（违反规则 A14 5 层）
8. ❌ 改共用组件不验证全部调用方（违反规则 A14）
9. ❌ LLM 链路改动不跑 benchmark 就 merge（违反规则 A13）
10. ❌ 覆盖率不达标就 merge（违反规则 A4）
11. ❌ 性能基线退化超阈值不 RCA 就 merge（违反规则 A7）
12. ❌ 用户说"我来 review" 就跳过 AI 双路评审（spec workflow 阶段 ② review spec 必须三路：codex + claude code-reviewer agent + 用户，缺一阻 merge。spec-0.4 教训。完整规则见设计仓 CLAUDE.md 规则 3）
